...
public:
	 ...
	 * @brief 设置文字边距信息
	 * @param[in] rc 边距信息
	 * @param[in] bNeedDpiScale 是否需要 DPI 缩放，默认为 true
	 * @return 无
	 */
	void SetTextPadding(UiRect rc, bool bNeedDpiScale = true);
	...
	/**
	 * @brief 重写 ReapplyDpi 以处理 Label 特有属性
	 * @return 无
	 */
	virtual void ReapplyDpi() override;
...
protected:
	...
	int		m_hAlign;
	int		m_vAlign;
	UiRect	m_rcTextPadding;           // 当前缩放后的文字边距
	UiRect	m_rcOriginalTextPadding;   // 原始未缩放的文字边距
	std::wstring	m_sText;
...
template<typename InheritType>
void LabelTemplate<InheritType>::SetTextPadding(UiRect rc, bool bNeedDpiScale)
{
	if (bNeedDpiScale) {
		// 保存原始值
		m_rcOriginalTextPadding = rc;
		// 缩放后保存
...
template<typename InheritType>
void LabelTemplate<InheritType>::ReapplyDpi()
{
	// 先调用基类的 ReapplyDpi
	__super::ReapplyDpi();

	// 重新应用 DPI 缩放到 TextPadding
	if (m_rcOriginalTextPadding.left != 0 || m_rcOriginalTextPadding.top != 0 ||
		m_rcOriginalTextPadding.right != 0 || m_rcOriginalTextPadding.bottom != 0) {
		UiRect scaledPadding = m_rcOriginalTextPadding;
		DpiManager::GetInstance()->ScaleRect(scaledPadding);
		// 不再次缩放，直接设置
		SetTextPadding(scaledPadding, false);
	}
}
