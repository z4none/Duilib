...
public:
	...
	/**
	 * @brief 设置控件原始固定宽度（未缩放）
	 * @param[in] cx 原始宽度
	 * @return 无
	 */
	void SetOriginalFixedWidth(int cx);

	/**
	 * @brief 获取控件原始固定宽度（未缩放）
	 * @return 原始宽度
	 */
	int GetOriginalFixedWidth() const;
	...
	/**
	 * @brief 设置控件原始固定高度（未缩放）
	 * @param[in] cy 原始高度
	 * @return 无
	 */
	void SetOriginalFixedHeight(int cy);

	/**
	 * @brief 获取控件原始固定高度（未缩放）
	 * @return 原始高度
	 */
	int GetOriginalFixedHeight() const;
	...
	/**
	 * @brief 重新应用 DPI 缩放（DPI 变化时调用）
	 * @return 无
	 */
	virtual void ReapplyDpi();
...
protected:
	...
	Window *m_pWindow;
	Box* m_pParent;
	CSize m_cxyFixed;            // 当前缩放后的固定大小
	CSize m_cxyOriginalFixed;    // 原始未缩放的固定大小
	CSize m_cxyMin;
	CSize m_cxyMax;
	UiRect m_rcItem;
